...
//							});
	...
	public void event_pop_up(int f, int r, FlexTable e) {
		...
		// for debug purpose, since without initilizing value, it will throw
		// exception
		else if (flag == 0) {
			String t = "";
		...
		save_d.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				...
				if (flag == 0) {
					...
					// //event_table.setWidget(row_temp, 0, new
					// Label("New ID generated by Server"));
					// event_table.setWidget(row_temp, 1, new
					// Label(event_title.getText()));
					// event_table.setWidget(row_temp, 2, new
					// Label(start_date.getValue().toString()));
					// event_table.setWidget(row_temp, 3, new
					// Label(end_date.getValue().toString()));
